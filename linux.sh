...
sudo sh ./crouton -t xiwi,unity-desktop,touch,keyboard,core,chrome,cli-extra,extension,audio,xorg -n linux -p '/media/removable/SD Card';
...
sudo enter-chroot -n linux sudo -S sh -c "$( curl -s https://code.headmelted.com/installers/apt.sh )";
if [ $? -eq 0 ]; then
...
echo "Aliasing 'code'..."
sudo echo "alias code=\"sudo startxiwi -n linux ${CODE_EXECUTABLE_NAME}\"" >> ~/.bashrc;
echo "Done!"
